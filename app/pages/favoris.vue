...
const { data: favoris, pending, error, refresh } = await useFetch('/api/favoris', {
  default: () => []
})
...
async function toggleFavori(idFilm: number) {
  try {
    await $fetch(`/api/favoris?filmId=${idFilm}`, { method: 'DELETE' })
  ...
  <div class="min-h-screen p-6 md:p-12">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
        <div>
          <h1 class="text-4xl font-extrabold text-white tracking-tight uppercase">Mes Favoris</h1>
          <p class="text-white/40 mt-2">Retrouvez ici tous les films que vous avez aimés.</p>
        </div>
        <NuxtLink to="/dashboard" class="text-white/20 hover:text-white/60 text-sm transition-colors flex items-center gap-2">
      ...
      <div v-else-if="error" class="glass glass-panel glass-red p-12 text-center rounded-3xl max-w-xl mx-auto">
        <h2 class="text-2xl font-bold text-white mb-3">Impossible de charger vos favoris</h2>
      ...
      <div v-else-if="!favoris || favoris.length === 0" class="glass glass-panel glass-red p-12 text-center rounded-3xl max-w-xl mx-auto">
        <h2 class="text-2xl font-bold text-white mb-3">Aucun favori pour le moment</h2>
        <p class="text-white/40 mb-8">Parcourez les films du moment et cliquez sur le cœur pour les ajouter ici.</p>
          ...
          <div class="aspect-[2/3] w-full relative">
            ...
            <div class="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-4">
              ...
              <div class="flex flex-col gap-2">
                <NuxtLink :to="`/movie/${movie.id}`" class="glass glass-btn text-[10px] py-2 text-center text-white uppercase tracking-wider">
                  Détails
                </NuxtLink>
                <button
                    @click="toggleFavori(movie.id)"
