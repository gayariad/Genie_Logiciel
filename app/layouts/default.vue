<template>
  <div
    class="relative h-screen text-white overflow-hidden flex flex-col"
    :class="hideHeader ? 'bg-[#1a0000]' : 'bg-animated-red'"
  >
    <div
    ...
    >
      ...
      <div class="flex items-center gap-3">
        <template v-if="loggedIn">
          <NuxtLink
            to="/profil"
            class="flex items-center gap-2 glass glass-pill px-4 py-1.5 text-sm text-white/60 hover:text-white transition-colors"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="w-4 h-4"
    ...
    <nav
      class="fixed left-5 top-1/2 -translate-y-1/2 z-40 flex flex-col items-center gap-3 py-5 px-3 w-16 glass glass-sidebar"
    >
      ...
      <button
        ...
        class="sidebar-icon"
        title="Profil"
        @click="openAuthPopoverFromSidebar"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="w-5 h-5"
    ...
    <Transition name="auth-popover-fade">
      ...
      >
        ...
        <div
          ...
          @click.stop
        >
          <p class="text-white/35 text-[11px] uppercase tracking-[0.22em] mb-2">
            Connexion requise
          </p>
          <h3 class="text-xl font-extrabold text-white tracking-wide">
            {{ authMode === "login" ? "Se connecter" : "Créer un compte" }}
          </h3>
          <p class="text-white/45 text-xs mt-1">
            Connectez-vous pour accéder à votre profil et à vos actions.
          </p>
          ...
          <form @submit.prevent="submitAuthPopover" class="mt-4 space-y-3">
            <div v-if="authMode === 'register'">
              <label
                class="block text-xs font-semibold text-white/50 uppercase tracking-wider mb-2"
                >Pseudo</label
              >
              <input
                v-model="authPseudo"
    ...
    <main
      ref="mainRef"
      class="relative z-10 pl-24 pr-8 pb-8 overflow-y-auto flex-1 min-h-0 snap-y snap-mandatory scroll-smooth"
    >
      <slot />
    </main>
  </div>
...
const authPopoverStyle = ref<Record<string, string>>({
  top: "12px",
  left: "12px",
});
...
async function submitAuthPopover() {
  ...
  try {
    ...
    authErrorMsg.value =
      e.data?.message || e.data?.statusMessage || "Erreur d'authentification";
  } finally {
    authLoading.value = false;
  }
}
